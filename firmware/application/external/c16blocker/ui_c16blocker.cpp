...
C16BlockerView::C16BlockerView(NavigationView& nav) : nav_{nav} {
    ...
    fld_freq_.set_value(433'920'000);
    fld_thr_.set_value(-60);
    fld_txg_.set_value(40);

    btn_brws_.on_select = [this](Button&) {
        auto view = nav_.push<FileLoadView>(".C16");
        view->on_changed = [this](std::filesystem::path p) {
            c16_path_ = p;
            ...
            push_log(file_ok_ ? "Loaded: " + name : "ERR open file");
            set_dirty();
        };
    };
    ...
    push_log("C16 Blocker+Rec ready");
    push_log("Set freq+threshold");
    push_log("Press SCAN+ARM");
}
...
void C16BlockerView::paint(Painter& painter) {
    ...
    // Separator lines
    painter.draw_hline({0, UI_POS_Y(2)},  screen_width, Theme::getInstance()->bg_darkest->foreground);
    painter.draw_hline({0, UI_POS_Y(7)},  screen_width, Theme::getInstance()->bg_darkest->foreground);
    painter.draw_hline({0, UI_POS_Y(9)},  screen_width, Theme::getInstance()->bg_darkest->foreground);
    painter.draw_hline({0, UI_POS_Y(11)}, screen_width, Theme::getInstance()->bg_darkest->foreground);

    // RSSI bar — 6px tall at row 2
    int bar_y = UI_POS_Y(2) + 2;
    painter.fill_rectangle({0, bar_y, screen_width, 6}, Theme::getInstance()->bg_darkest->foreground);

    int norm = rssi_val_ + 120;
    if (norm < 0)   norm = 0;
    if (norm > 120) norm = 120;
    int bar_w = norm * screen_width / 120;

    Color bar_col = (state_ == AppState::DETECTED) ? Theme::getInstance()->fg_red->foreground
                  : (state_ == AppState::SCANNING)  ? Theme::getInstance()->fg_green->foreground
                  : Theme::getInstance()->fg_yellow->foreground;

    if (bar_w > 0)
        painter.fill_rectangle({0, bar_y, bar_w, 6}, bar_col);
}
...
void C16BlockerView::set_state(AppState s) {
    ...
    case AppState::DETECTED:
        ...
        txt_status_.set("DETECTED! JAM+REC active");
    ...
    case AppState::SENDING:
        ...
        txt_status_.set("Sending C16 file...");
...
void C16BlockerView::do_arm() {
    ...
    push_log("Armed @ " +
        to_string_dec_uint((uint32_t)(fld_freq_.value() / 1'000'000)) + "MHz");
}

void C16BlockerView::do_send() {
    if (!file_ok_) { push_log("No C16 loaded!"); return; }
    if (sending_)  { stop_replay(); sending_ = false; set_state(AppState::IDLE); return; }
    if (state_ == AppState::DETECTED) return;

    sending_ = true;
    start_tx(fld_freq_.value());
    start_replay(fld_freq_.value());
...
void C16BlockerView::start_rx(rf::Frequency freq) {
    ...
    receiver_model.set_sampling_rate(SAMPLE_RATE);
    receiver_model.set_baseband_bandwidth(BW);
    receiver_model.enable();
}

void C16BlockerView::start_tx(rf::Frequency freq) {
    receiver_model.disable();
...
void C16BlockerView::start_capture(rf::Frequency freq) {
    rec_name_ = "det_" + to_string_dec_uint((uint32_t)(freq / 1000)) + ".C16";
    auto path = std::filesystem::path(u"/CAPTURES") / rec_name_;

    auto writer = std::make_unique<FileWriter>();
    auto err = writer->create(path);
    if (err.is_valid()) {
        push_log("SD write err!");
        return;
    }
...
void C16BlockerView::start_replay(rf::Frequency freq) {
    (void)freq;
    if (!file_ok_) return;

    // Try jam_noise.C16 first, fall back to loaded file
    std::filesystem::path jam_path{u"/CAPTURES/jam_noise.C16"};
    File chk;
    auto err = chk.open(jam_path);
    auto& path = err.is_valid() ? c16_path_ : jam_path;

    auto reader = std::make_unique<FileReader>();
    if (reader->open(path).is_valid()) {
        push_log("ERR open replay file");
        return;
    }

    bool dummy_ready = false;
...
void C16BlockerView::on_signal_detected() {
    rf::Frequency freq = fld_freq_.value();
    push_log("DETECT " +
        to_string_dec_uint((uint32_t)(freq / 1'000'000)) + "." +
        to_string_dec_uint((uint32_t)((freq % 1'000'000) / 1000)) +
